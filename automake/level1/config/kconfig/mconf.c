...
int main(int ac, char **av)
{
    ...
    sym = sym_lookup(proj_version_name, 0);
    sym_calc_value(sym);
    sprintf(menu_backtitle, _("%s %s Configuration"), proj_name_cap,
        sym_get_string_value(sym));
    ...
    do {
        cprint_init();
        cprint("--yesno");
        cprint(_("Do you wish to save your new %s configuration?"), proj_name_cap);
        cprint("5");
        cprint("60");
        stat = exec_conf();
    } while (stat < 0);
    ...
    if (stat == 0) {
        if (conf_write(NULL)) {
            fprintf(stderr, _("\n\n"
                "Error during writing of the %s configuration.\n"
                "Your %s configuration changes were NOT saved."
                "\n\n"), proj_name_cap, proj_name_cap);
            return 1;
        }
        printf(_("\n\n"
            "*** End of %s configuration.\n"
            "*** Execute 'make' to build %s or try 'make help'."
            "\n\n"), proj_name_cap, proj_name_cap);
    } else {
        fprintf(stderr, _("\n\n"
            "Your %s configuration changes were NOT saved."
